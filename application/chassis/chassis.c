...
static Publisher_t *UI_Referee_pub; // 用于发布底盘的数据
...
static Referee_Upload_Data_s In_Chsaais_Referee_To_Vision; // 给视觉的信息
...
void ChassisInit()
{
    ...
    PowerMeter_Init_Config_s power_meter_config = {
        .can_config = {
            .can_handle = &hcan1,
            .tx_id = 0x300,
            .rx_id = 0x301,
    ...
    power_meter = PowerMeterInit(&power_meter_config); // 功率计初始化
    // 发布订阅初始化,如果为双板,则需要can comm来传递消息
#ifdef CHASSIS_BOARD
    // Chassis_IMU_data = INS_Init(); // IMU初始化移至StartINSTASK,避免init阶段PWM噪声
...
#ifdef ONE_BOARD
    rc_data = RemoteControlInit(&huart5); // 仅单板在此初始化遥控器,双板时遥控器插在云台板上
#endif
    ...
    UI_Referee_pub = PubRegister("UI_feed", sizeof(Referee_Upload_Data_s));
...
static void LimitChassisOutput()
{
    ...
    uint16_t tempPower = chassis_power_recv.chassis_power_mx * 1000; // 感觉没有使用
    uint8_t data[8];
    data[0] = tempPower >> 8;
    data[1] = tempPower;
    SuperCapSend(cap, data);
    // ★ 如果没收到 power_cmd，用默认电流上限（否则 MaxOut=0 电机不转）
    if (chassis_power_recv.motor_current_up[0] == 0) {
        // 没收到功率限制数据，设默认值
        motor_lf->motor_controller.current_PID.MaxOut  = 3000;
        motor_lf->motor_controller.current_PID.MaxOut_ = -3000;
        motor_rf->motor_controller.current_PID.MaxOut  = 3000;
        motor_rf->motor_controller.current_PID.MaxOut_ = -3000;
        motor_lb->motor_controller.current_PID.MaxOut  = 3000;
        motor_lb->motor_controller.current_PID.MaxOut_ = -3000;
        motor_rb->motor_controller.current_PID.MaxOut  = 3000;
        motor_rb->motor_controller.current_PID.MaxOut_ = -3000;
    } else {
        // 有功率限制数据时，用原本的逻辑
...
void ChassisTask()
{
    ...
    // ====== 在 L240 行（SubGetMessage(gimbal_feed_sub, ...)）之后插入 ======

    // // ★ 底盘测试硬编码：覆盖默认的全零值
    // chassis_cmd_recv.chassis_mode = CHASSIS_NO_FOLLOW;  // 改为非零力模式
    // chassis_cmd_recv.vx = 5000;   // 前进速度，单位 °/s；改这个值调速度
    // chassis_cmd_recv.vy = 0;      // 左右
    // chassis_cmd_recv.wz = 0;      // 旋转
    // ★ 遥控器控制
    if (switch_is_up(rc_data[TEMP].rc.switch_left)) {
        ...
        // chassis_cmd_recv.chassis_mode = CHASSIS_NO_FOLLOW;
            ...
            chassis_cmd_recv.chassis_mode = CHASSIS_NO_FOLLOW;
        chassis_cmd_recv.vx = -26.0f * (float)rc_data[TEMP].rc.rocker_l_;
        chassis_cmd_recv.vy = -26.0f * (float)rc_data[TEMP].rc.rocker_l1;
        chassis_cmd_recv.wz = +15.0f * (float)rc_data[TEMP].rc.rocker_r_;
...
// ====== 插入结束，下面不改 ======
    ...
    {
        if (prev_mode != CHASSIS_ZERO_FORCE)
        {
    ...
    prev_mode = chassis_cmd_recv.chassis_mode;
    ...
    In_Chsaais_Referee_To_Vision.color = referee_data->referee_id.Robot_Color;
    In_Chsaais_Referee_To_Vision.allowance_17mm = referee_data->ProjectileAllowance.projectile_allowance_17mm;
    In_Chsaais_Referee_To_Vision.game_progress = referee_data->GameState.game_progress;
